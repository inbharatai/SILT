...
Writes ONLY as ``python scripts/setup_local_validation.py``: the whole body
sits under a __main__ guard (audit 2026-09-18), so importing or collecting
this module (test discovery, help tooling) has no filesystem side effects --
previously an import wrote fixtures and parsed sys.argv.
"""
import argparse
import json
...
def main():
    ...
    a = p.parse_args()
    root = Path(a.models_root).resolve()
