...
**Evidence kind:** Implemented (source_code_verified) with offline mechanism tests plus an executed judged pilot (2026-09-18: 19-case / 54-check five-split dataset built and frozen; 16 non-final cases traced against Ollama-cloud `glm-5.3-flash:cloud` under per-run consent; host-oracle pass rates on identical target checks — teacher 0.8649, local student 0.4595 — a measurement of this case set only, not a model-quality claim) and a DeepApply training stage (2026-09-19: real LoRA through the production trainer, independent host-oracle A/B; CORRECTED scoring, same-day external audit — the published rates divided by reported judged totals so candidate-error cases vanished from the denominators; under authored-denominator accounting the adapter REGRESSED the measured target capability: heldout 0.8889→0.6667, aggregate target checks 0.8108→0.5405, retention 0.625 — recorded as the product, nothing admitted). Three signed receipts live in the append-only store (the third corrects and invalidates the second by its stored hash); a signed receipt is tamper evidence, not a capability certificate.
...
**Invoke / knobs:** `silt-capability trace --mode internal --checkpoint DIR` / `intervene --component expert:3/7 --cases CASES.json --checkpoint DIR` (cases carry per-case oracle frames; judging runs through the host oracle, so the command needs the Linux sandbox and fails closed on Windows-native runs); worker (`workers/glm53/`) has its own Dockerfile and `requirements.lock` (**transformers 5.16.1** + accelerate 1.15.0 — the model card DECLARES 5.16.0, but that wheel does not ship `models/glm5_next`; 5.16.1 is the minimal release that does, the image's BUILD-TIME smoke test proves the architecture module/mapping/accelerate are present in the pinned wheels, and `hello` records both the declared and the installed version — external audit 2026-09-19; the core `transformers==4.51.3` pin is untouched; the GLM runtime never shares an env with Qwen/Switch).

**Mechanism:** Net-new protocol (the compiler only prunes physically; no temporary-masking path existed): verify-unchanged → seeded baseline → temporary mask → measure → restore → verify-unchanged again. The mask is a router-output forward hook — no weight row is ever written (writing a constant into a router Linear's row yields `−c·Σ(hidden)`, strongly positive for negative-sum hidden states, which would have made the masked expert MORE likely to be routed). **Audit 2026-09-19 corrections, verified against the real transformers 5.16.1 source:** the v5.16.1 router (`Glm5NextTextTopkRouter`) returns the full dispatch tuple `(router_logits, topk_weights, topk_indices)` computed as sigmoid scores + `e_score_correction_bias` on the SELECTION scores only, group top-k, pre-bias weight gather, routed scaling — so the mask hook rewrites the FULL tuple, forcing the masked expert to `-inf` in the selection scores BEFORE the group stage (a nonzero correction bias added after a `−1e9` logit could otherwise push the masked expert back into the top-8; the old hook rewrote only `output[0]` and masked interventions silently measured an UNMASKED teacher), telemetry captures the ACTUAL dispatched ids/weights from the router's own output (never a re-derived plain-sigmoid approximation; the old code refused a config boolean `e_score_correction_bias` that the real config never had), detection verifies the ROUTER MODULE CONTRACT (weight shape, bias buffer, group/scaling attributes) instead of the nonexistent `scoring_func` config field, telemetry and mask hooks refuse to be live together (the telemetry hook registered first would record the unmasked dispatch as if masked), the worker loads through `AutoModelForMultimodalLM` (there is no `Glm5NextForCausalLM` in 5.16.1) with an arch-presence check BEFORE any weights are touched and checkpoint-identity sha256s (config.json / generation_config.json / model index) reported in hello/inspect, and the worker exposes a `generate` op (completions under the CURRENT mask state, live masks reported with every generation) that closes the intervention loop: `intervene` regenerates each case through the worker and grades it through the host oracle in both arms, refusing by name when the worker's reported mask set contradicts the phase being scored. Byte-exact parameter-hash verification; an unrestorable intervention is **never** causal evidence. Teachers are read-only; interventions refuse quantized checkpoints (main HF repo is FP8; remedy names the BF16 variant). Versioned JSONL frames; worker memory preflight runs before load; crash preserves partial frames and never fabricates the lost portion. The client ENFORCES request deadlines over the whole exchange (watchdog armed before transmission; a typed `WorkerTimeout` carries preserved frames and the drained stderr tail). The worker image's build context ships `build_support.py` AND every `AUDIT_ORIGINS` input the wheel build hook snapshots — the hook fails on any missing input, so `pip install /silt` inside the image succeeds with the full audit trail embedded; a clean image build was verified on this machine on 2026-09-18.

**Limits:** On small hosts the preflight admits the teacher as `BLOCKED_RESOURCE` — the correct honest outcome, not a bug (a ~320B-parameter teacher does not fit this class of hardware). The loop is wired in code but **no GLM intervention has been measured**: the worker has never run on teacher-sized hardware, so every live invocation reports the honest hardware block; the end-to-end mechanism is proven on faithful-replica fixtures only. Routing frequency is never called causal importance. The worker-side structural `reduce` operation does not exist in this build (the adapter refuses by name rather than pointing at a nonexistent handler, and no GLM reduction has been executed anywhere).
...
**Default / enablement:** implemented; explicit invocation. Student connectors are LOCAL only — the student host URL is parsed and its actual hostname validated (literal loopback only; lookalike hosts such as `http://localhost.example.invalid:11434` are a typed refusal), and redirects are refused at the TRANSPORT level (the shared `urllib` opener rejects 3xx instead of following them, tested through the actual connector — a redirecting local daemon can never smuggle a request off-host); no remote student path exists.

**Invoke / knobs:** `silt-capability dataset build --spec spec.json --cases X.jsonl --out data/capability_v1` / `dataset validate --dir`; `student-baseline` (local Ollama); `distill --spec spec.json --dataset data/capability_v1`.

**Mechanism:** Five splits (training/development/heldout/final/controls) with per-case ID, content hash, family, provenance, license; cross-split ID/content/family disjointness; a NEW near-duplicate token-shape guard; the frozen September sets quarantined as inputs; manifest+selection-lock frozen before any model sees a case. The spec is REQUIRED and the manifest binds capability ID, teacher identity and the spec fingerprint; `dataset validate` refuses an altered identity, a MISSING selection lock (a dataset without its lock was never frozen by a build) and a manifest that no longer matches the lock's `manifest_sha256` (edited after freezing). Student baselines measure the gap `Gap = TeacherScore − StudentScore` — the only justification for transfer work. `distill` REQUIRES the built dataset, verifies its identity against the spec, builds protected sets (IDs, content hashes, prompt hashes, prompt token SHAPES, families) from every non-training split, and refuses any trace outside the training split — and a training sample ID is an IDENTITY CLAIM, not a free pass: each behavioural trace's prompt must equal its approved training row byte for byte, so a trace filed under a training ID with a rewritten prompt (including a protected prompt rewritten only in capitalisation or punctuation) is refused. KD pairs are built ONLY from host-judged successful traces (failures stay visible as labelled negatives; UNJUDGED contributes nothing; one leakage collision refuses the whole set) and hands training to DeepApply (LoRA via Gate 2); cross-family pairs are text-only — no vocabulary/logit alignment assumed.
...
**Evidence kind:** Implemented + mechanism-tested offline, then executed against real model material in the GLM-5.3-Flash pilot: the five-split dataset was really built and frozen (selection lock + manifest hash pins), a real local student baseline was host-oracle-judged (16 cases), `distill` really built 6 byte-bound KD pairs from judged training-split traces, and a real DeepApply training run consumed them through the production trainer (research intake, honestly labelled — production admission was NOT sought; see [CAPABILITY_BUILD.md](CAPABILITY_BUILD.md) for the full chain and the mixed A/B verdict). Building and training admit nothing.
...
**Invoke / knobs:** `asea.capability_build.search.search_with_reference(spec, teacher_target_score=…, teacher_control_baselines=…, candidate_plan=…, build=…, evaluate=…)`.

**Mechanism:** `min Size(M)` s.t. `TargetScore(M) ≥ minimum_retention_ratio × TargetScore(teacher)` and control regression ≤ tolerance against MEASURED teacher control baselines (refuses to run on an empty or assumed-1.0 baseline set); every candidate's target score, control results and `size_bytes` must be measured AND FINITE — a NaN control score (which passes every regression comparison, i.e. shows "zero regression") and an infinite target score (which satisfies any threshold) are refused before arithmetic, as are non-finite teacher scores and baselines; the storage budget is enforced, the ENTIRE plan is evaluated (never stops at the first passing candidate), the smallest passing candidate by measured size is selected, and every iteration — including rejects with explicit reasons and failures — is recorded; accepted candidates carry `admission: CANDIDATE_UNADMITTED`.
