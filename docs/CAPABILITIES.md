...
**Mechanism:** Five splits (training/development/heldout/final/controls) with per-case ID, content hash, family, provenance, license; cross-split ID/content/family disjointness; a NEW near-duplicate token-shape guard; the frozen September sets quarantined as inputs; manifest+selection-lock frozen before any model sees a case. The spec is REQUIRED and the manifest binds capability ID, teacher identity and the spec fingerprint; `dataset validate` refuses an altered identity. Student baselines measure the gap `Gap = TeacherScore − StudentScore` — the only justification for transfer work. `distill` REQUIRES the built dataset, verifies its identity against the spec, builds protected sets (IDs, content hashes, prompt hashes, families) from every non-training split, and refuses any trace outside the training split. KD pairs are built ONLY from host-judged successful traces (failures stay visible as labelled negatives; UNJUDGED contributes nothing; one leakage collision refuses the whole set) and hands training to DeepApply (LoRA via Gate 2); cross-family pairs are text-only — no vocabulary/logit alignment assumed.
...
**Mechanism:** `min Size(M)` s.t. `TargetScore(M) ≥ minimum_retention_ratio × TargetScore(teacher)` and control regression ≤ tolerance against MEASURED teacher control baselines (refuses to run on an empty or assumed-1.0 baseline set); every candidate's target score, control results and `size_bytes` must be measured, the storage budget is enforced, the ENTIRE plan is evaluated (never stops at the first passing candidate), the smallest passing candidate by measured size is selected, and every iteration — including rejects with explicit reasons and failures — is recorded; accepted candidates carry `admission: CANDIDATE_UNADMITTED`.
